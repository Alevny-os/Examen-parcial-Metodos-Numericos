import numpy as np
from scipy.interpolate import CubicSpline

# -----------------------------------------
# Datos experimentales
# -----------------------------------------

f = np.array([
    100,120,145,170,200,235,270,310,355,405,
    460,520,585,655,730,810,895,985,1080,1180,
    1290,1410,1540,1680,1830,1990,2160,2340,
    2530,2730
])

Z = np.array([
    152.3,149.1,146.8,144.9,142.0,139.5,137.9,
    136.1,134.8,133.6,132.7,131.9,131.4,131.1,
    130.9,131.0,131.3,131.9,132.7,133.8,135.2,
    136.9,138.9,141.1,143.5,146.1,149.0,152.2,
    155.6,159.2
])

# -----------------------------------------
# Spline cúbico natural
# -----------------------------------------

S = CubicSpline(
    f,
    Z,
    bc_type='natural'
)

# Segunda derivada
segunda_derivada = S.derivative(2)

# Frecuencia aproximada del mínimo
f_minimo = 742.1585

# Evaluación
valor_segunda = segunda_derivada(f_minimo)

print("\nSEGUNDA DERIVADA EN EL MÍNIMO\n")

print(
    f"d²|Z|/df²({f_minimo:.4f}) "
    f"= {valor_segunda:.6f} ohm/Hz²"
)

# Interpretación simple
if valor_segunda > 0:
    print("\nLa segunda derivada es positiva.")
    print("Esto confirma la presencia de un mínimo local estable.")

else:
    print("\nLa segunda derivada no es positiva.")